#!/bin/bash
EXP_NAME=distill_finetuned_base_model
GPUS=4
SAVE_DIR1="./work_dirs/${EXP_NAME}_inaturalist_e2s1_2sameT/"

IMAGENET_DIR='/home/ps/scratch/KD_imbalance/BalancedKnowledgeDistillation/data/inaturalist-2019/all'

FINETUNE_EXP_FOLDER='distill_base_model_inaturalist_e2s1_2sameT'
FINETUNE_MODEL_NAME='checkpoint-99.pth'


OMP_NUM_THREADS=1 python -m torch.distributed.launch --nproc_per_node=${GPUS} main_finetune.py \
    --output_dir ${SAVE_DIR1} \
    --log_dir ${SAVE_DIR1} \
    --batch_size 128 \
    --model vit_base_patch16 \
    --finetune "./work_dirs/${FINETUNE_EXP_FOLDER}/${FINETUNE_MODEL_NAME}" \
    --epochs 100 \
    --blr 5e-4 \
    --weight_decay 0.05 --mixup 0.8 --cutmix 1.0 --reprob 0.25 \
    --dist_eval \
    --data_path ${IMAGENET_DIR} \
    --seed 0 --nb_classes 1010
