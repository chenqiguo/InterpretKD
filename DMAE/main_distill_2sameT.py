...
import models_mae_distill

from engine_distill_2sameT import train_one_epoch_2sameT

# newly added by Chenqi:
from util import dataloader_LT


# as one of the baseline (ablation study) for our proposed method,
# this dummy code do the following things in this pre-training stage:
# init 2 same teacher models (both trained on the whole dataset),
# then distil the mean of their intermediate features to the student model.
...
def main(args):
    ...
    # simple augmentation: newly modified by Chenqi:
    ...
    if 'ILSVRC' in args.data_path and 'LT' in args.output_dir:
        ...
        dataset_train = dataloader_LT.load_data(data_root=args.data_path, dataset='ImageNet-LT', phase='train')
    
    if 'cifar-10-' in args.data_path:
        if 'imb100/' in args.output_dir:
            print('--> For CIFAR-10 imb100 dataset')
            dataset_train = dataloader_LT.load_data(data_root='', dataset='CIFAR-10 imb100', phase='train')
        elif 'imb50/' in args.output_dir:
            print('--> For CIFAR-10 imb50 dataset')
            dataset_train = dataloader_LT.load_data(data_root='', dataset='CIFAR-10 imb50', phase='train')
        elif 'imb10/' in args.output_dir:
            print('--> For CIFAR-10 imb10 dataset')
            dataset_train = dataloader_LT.load_data(data_root='', dataset='CIFAR-10 imb10', phase='train')
    
    if 'cifar-100-' in args.data_path:
        if 'imb100/' in args.output_dir:
            print('--> For CIFAR-100 imb100 dataset')
            dataset_train = dataloader_LT.load_data(data_root='', dataset='CIFAR-100 imb100', phase='train')
        
    if 'cifar-100-' in args.data_path:
        if 'imb50/' in args.output_dir:
            print('--> For CIFAR-100 imb50 dataset')
            dataset_train = dataloader_LT.load_data(data_root='', dataset='CIFAR-100 imb50', phase='train')
     
    if 'cifar-100-' in args.data_path:
        if 'imb10/' in args.output_dir:
            print('--> For CIFAR-100 imb10 dataset')
            dataset_train = dataloader_LT.load_data(data_root='', dataset='CIFAR-100 imb10', phase='train')
    ...
    print('************* HERE2 !')
    """
    # Define 2 same teacher model
    model_teacher_1 = models_mae_distill.__dict__[args.model_teacher](
        norm_pix_loss=args.norm_pix_loss,
        img_size=args.input_size,
        embedding_distillation_func=args.embedding_distillation_func,
    ...
    model_teacher_2 = models_mae_distill.__dict__[args.model_teacher](
        norm_pix_loss=args.norm_pix_loss,
        img_size=args.input_size,
        embedding_distillation_func=args.embedding_distillation_func,
        aligned_blks_indices=args.aligned_blks_indices if args.teacher_aligned_blks_indices is None
    ...
    """
    # for debug:
    print('************* init model_teacher:')
    print(model_teacher)
    """
    model_teacher_1.to(device)
    model_teacher_2.to(device)

    model_teacher_without_ddp_1 = model_teacher_1
    model_teacher_without_ddp_2 = model_teacher_2
    print("Teacher Model = %s" % str(model_teacher_without_ddp_1))
    ...
    if args.distributed:
        ...
        model_teacher_1 = torch.nn.parallel.DistributedDataParallel(model_teacher_1, device_ids=[args.gpu],
                                                                  find_unused_parameters=True)
        model_teacher_without_ddp_1 = model_teacher_1.module
        
        model_teacher_2 = torch.nn.parallel.DistributedDataParallel(model_teacher_2, device_ids=[args.gpu],
                                                                  find_unused_parameters=True)
        model_teacher_without_ddp_2 = model_teacher_2.module
    ...
    misc.load_model_teacher(args=args, model_teacher_without_ddp=model_teacher_without_ddp_1)
    misc.load_model_teacher(args=args, model_teacher_without_ddp=model_teacher_without_ddp_2)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_2sameT(
            model, model_teacher_1, model_teacher_2, data_loader_train,
            optimizer, device, epoch, loss_scaler,
            log_writer=log_writer,
            args=args
        )
        if args.output_dir and (epoch % 50 == 0 or epoch + 1 == args.epochs): # % 5
            misc.save_model(
